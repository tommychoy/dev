...
$ISO8601TimeSpan = Get-QueryTimeSpan -StartUTCTime $CurrentProcessingUTCTime -EndUTCTime $CurrentProcessingUTCTime.AddDays(1)
...
#Process search count result
  If($SearchCount -eq -1){
      ...
      While ($TotalRetrievedCount -lt $SearchCount){
        ...
        $FileName = "$SearchCategory-Daily-$($ISO8601TimeSpan.replace(':', '.').split('T')[0])-file$queryiteration`.JSON" 
        $OutputFilePath = Join-Path $FolderName $FileName
        Write-Output "Temp file path: $OutputFilePath"
      ...
      if($arrLogFiles){
          ...
          #Zip the JSON files
          $ZipFileName = "$SearchCategory-Daily-$($ISO8601TimeSpan.replace(':', '.').split('T')[0])`.zip" 
   ...
   $CurrentProcessingUTCTime = $CurrentProcessingUTCTime.AddDays(1)
